function PlotTheTopo2(Slopes, Slopes2, GroupsTopoplot, Chanlocs, PlotProps, Logical, Logical2, Logical3, Logical4, lastrow)
...
for i = 1:108
        [~, p, ~, ~] = ttest2(squeeze(Slopes(i, (Logical&Logical3))), squeeze(Slopes(i, (Logical2&Logical3))));
        stats1(i) = p;
        [~, p, ~, ~] = ttest2(squeeze(Slopes(i, (Logical&Logical4))), squeeze(Slopes(i, (Logical2&Logical4))));
        stats2(i) = p;
        [~, p, ~, ~] = ttest2(squeeze(Slopes(i, (Logical&Logical3))), squeeze(Slopes(i, (Logical&Logical4))));
        stats3(i) = p;
        [~, p, ~, ~] = ttest2(squeeze(Slopes(i, (Logical2&Logical3))), squeeze(Slopes(i, (Logical2&Logical4))));
        stats4(i) = p;
end
...
% get groups
Group1 = vertcat(mean(Slopes(:, (Logical&Logical3)), 2), mean(Slopes(:,(Logical&Logical4)), 2));
Group2 = vertcat(mean(Slopes(:,(Logical2&Logical3)), 2), mean(Slopes(:,(Logical2&Logical4)), 2));
Group3 = (squeeze(mean(Slopes(:,(Logical&Logical3) ), 2)) - squeeze(mean(Slopes(:,(Logical2&Logical3)), 2)));
Group4 = (squeeze(mean(Slopes(:,(Logical&Logical4)), 2)) - squeeze(mean(Slopes(:,(Logical2&Logical4)), 2)));
